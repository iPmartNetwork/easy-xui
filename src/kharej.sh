...
if [[ "$choice" == "warp" ]]; then
    ...
    wget -q -O /etc/x-ui/x-ui.db https://github.com/iPmartNetwork/xui/raw/main/x-ui/db/warp.db  >/dev/null 2>&1
    systemctl restart x-ui >/dev/null 2>&1
    # Run setup_warproxy.sh script in the background without showing any output
    nohup curl -s https://raw.githubusercontent.com/iPmartNetwork/xui/main/warp/setup_warproxy.sh | bash >/dev/null 2>&1 
    echo ""
else
    echo "Configuring x-ui database for Direct..."
    wget -q -O /etc/x-ui/x-ui.db https://github.com/iPmartNetwork/xui/raw/main/x-ui/db/direct.db >/dev/null 2>&1
fi
